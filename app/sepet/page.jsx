...
//                     product_quantity: counts[item.id] || 0
//                 });
//         });
...
//                 text: 'Siparişiniz Supabase veritabanına eklendi.',
//                 icon: 'success',
//                 confirmButtonText: 'Tamam',
//             });
...
//         } catch (error) {
//             console.error("Sipariş kaydedilirken bir hata oluştu:", error.message);
//             Swal.fire({
//                 title: 'Hata!',
...
export default function Basket() {
    ...
    const [error, setError] = useState(null);
    const [counts, setCounts] = useState({});
    const [ordersData, setOrdersData] = useState([]); // Sipariş verilerini tutmak için yeni state

    useEffect(() => {
        fetchData();
        fetchOrders(); // Sayfa yüklendiğinde sipariş verilerini getir
    }, []);
    ...
    const fetchOrders = async () => {
        console.log('Sipariş verileri çekiliyor...');
        const { data, error } = await supabase
            .from('orders')
            .select('name_surname, address, product_name, product_price, product_quantity');

        if (error) {
            console.error('Sipariş verileri çekilirken hata oluştu:', error);
        } else {
            console.log('Sipariş verileri başarıyla çekildi:', data);
            setOrdersData(data); // Çekilen sipariş verilerini ordersData state'ine kaydediyoruz
        }
    };
    ...
    return (
        <div className="mega-container">
            <div className="basket-cotainer">
                <Link className="orders-btn" href="/orders">Siparişlerim</Link>
                <div className="card-container">
